...
void GetTagTable(std::deque<Value*>& All, std::string Patch, PGresult* res, int l, std::ofstream& ofs)
{
    for(auto& val : All)
    {
        if(val->Patch == Patch)
        {
            ...
            val->coeff = Stof(conn_spis.PGgetvalue(res, l, CollTag::Coeff).c_str());
            val->hist = Stof(conn_spis.PGgetvalue(res, l, CollTag::Hist).c_str());
            val->format = conn_spis.PGgetvalue(res, l, CollTag::Format);
            //if(!val->Sec)
				val->Sec = static_cast<MSSEC>(Stoi(conn_spis.PGgetvalue(res, l, CollTag::Idsec).c_str()));
...
#pragma region SELECT id, name, type, arhive, comment, content, coeff, hist, format, idsec FROM tag ORDER BY id 
    ...
    std::string command = "SELECT id, name, type, arhive, comment, content, coeff, hist, format, idsec FROM tag ORDER BY type, name;";
