...
namespace PDF
{
	...
	void PdfClass::SavePDF(size_t index)
	{
...
#pragma endregion
	...
	void GetPdf::EndCassette(PGConnection& conn, TCassette& P, int Petch, std::fstream& s1)
	{
		try
		{
			...
			if(S107::IsCassete(P) && P.Run_at.size())
			{
	...
	void GetPdf::SaveFileSdg(MapRunn& CasseteTodos)
	{
#ifdef _DEBUG
		try
		{
...
#pragma region Запись в файл Cass.csv тела
			...
			{
				if(!isRun)return;
				if(it.second.Run_at.length() && it.second.End_at.length() && S107::IsCassete(it.second))
				{
					s1 << it.first << ";"
						<< " " << it.second.Run_at << ";"
						<< " " << it.second.End_at << ";"
...
#pragma endregion
	...
	void GetPdf::GetCassete(PGConnection& conn, MapRunn& CasseteTodos, int Petch)
	{
		TCassette P;
		T_ForBase_RelFurn* Furn = NULL;
		try
		...
		try
		{
			...
			for(auto a : CasseteTodos)
			{
				...
				std::stringstream ssd;
				ssd << "GetCassete:CasseteTodos " << Petch << ", " << size << ":" << lin;
				SetWindowText(hWndDebug, ssd.str().c_str());
				...
				try
				{
					...
					{
						if(!P.Error_at.length() && S107::IsCassete(P))
						{
	...
	void GetPdf::GetCassete(PGConnection& conn)
	{
		MapRunn CasseteTodos;
		SetWindowText(hWndDebug, "SaveCassete");
		try
		{
			GetCassete(conn, CasseteTodos, 1);
			GetCassete(conn, CasseteTodos, 2);

			SaveFileSdg(CasseteTodos);
